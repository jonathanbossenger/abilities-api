...
- **Ability:** A distinct piece of functionality with a unique ID following the `namespace/ability-name` pattern. Each ability has a human-readable name and description, input/output definitions (using JSON Schema), optional permissions, and an associated callback function for execution.
...
- **Callback:** The PHP function or method executed when an ability is called via `wp_execute_ability()`.
...
- **Namespace:** The first part of an ability ID (before the slash), typically matching the plugin or component name that registers the ability.
